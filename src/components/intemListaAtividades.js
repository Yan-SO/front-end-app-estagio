...
export default function ItemListaAtividades({deletar,item}){
    ...
    if(deletar){
        return (
            <TouchableOpacity onPress={()=> navigation.navigate('ApagarAtividade',{item:atividade})} style={styles.conteiner}>
                <Text  style={styles.texto}>titulo: {atividade.titulo}</Text>
                <Text style={styles.texto}>pergunta: {atividade.pergunta}</Text>
                <Text style={styles.texto}>resposta: {atividade.resposta}</Text>
            </TouchableOpacity>
        );
    }
